defmodule Auction.UsersTest do
  use Auction.DataCase

  alias Auction.Users

  import Auction.UsersFixtures
  alias Auction.Users.{User, UserToken}
  ...
  describe "user_shortlist" do
    alias Auction.Users.UserShortlist

    import Auction.UsersFixtures

    @invalid_attrs %{shortlist_type: nil}

    test "list_user_shortlist/0 returns all user_shortlist" do
      user_shortlist = user_shortlist_fixture()
      assert Users.list_user_shortlist() == [user_shortlist]
    end

    test "get_user_shortlist!/1 returns the user_shortlist with given id" do
      user_shortlist = user_shortlist_fixture()
      assert Users.get_user_shortlist!(user_shortlist.id) == user_shortlist
    end

    test "create_user_shortlist/1 with valid data creates a user_shortlist" do
      valid_attrs = %{shortlist_type: "some shortlist_type"}

      assert {:ok, %UserShortlist{} = user_shortlist} = Users.create_user_shortlist(valid_attrs)
      assert user_shortlist.shortlist_type == "some shortlist_type"
    end

    test "create_user_shortlist/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Users.create_user_shortlist(@invalid_attrs)
    end

    test "update_user_shortlist/2 with valid data updates the user_shortlist" do
      user_shortlist = user_shortlist_fixture()
    ...
    test "delete_user_shortlist/1 deletes the user_shortlist" do
      user_shortlist = user_shortlist_fixture()
      assert {:ok, %UserShortlist{}} = Users.delete_user_shortlist(user_shortlist)
      assert_raise Ecto.NoResultsError, fn -> Users.get_user_shortlist!(user_shortlist.id) end
    end

    test "change_user_shortlist/1 returns a user_shortlist changeset" do
      user_shortlist = user_shortlist_fixture()
      assert %Ecto.Changeset{} = Users.change_user_shortlist(user_shortlist)
    end
  end
end
